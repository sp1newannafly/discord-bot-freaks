...
class aclient(discord.Client):
    ...
    def get_chatbot_model(self, prompt=prompt) -> Union[AsyncChatbot, Chatbot]:
        if self.chat_model == "НЕОФИЦИАЛЬНЫЙ":
        ...
        elif self.chat_model == "ОФИЦИАЛЬНЫЙ":
    ...
    async def send_message(self, message, user_message):
        ...
        try:
            ...
            if self.is_replying_all == "True":
                await message.channel.send("> **ОШИБКА:Что-то пошло не так, пожалуйста, повторите попытку позже!**")
            else:
                await message.followup.send("> **ОШИБКА: Что-то пошло не так, пожалуйста, повторите попытку позже!**")
            ...
            if os.path.isfile(prompt_path) and os.path.getsize(prompt_path) > 0:
                with open(prompt_path, "r", encoding="utf-8") as f:
                    ...
                    if (discord_channel_id):
                        logger.info(f"Отправьте системный запрос с указанием размера {len(prompt)}")
                        response = ""
                        if self.chat_model == "ОФИЦИАЛЬНЫЙ":
                        ...
                        logger.info(f"Ответ системы:{response}")
                    else:
                        logger.info("Канал не выбран. Пропуск отправки системного запроса.")
            else:
                logger.info(f"No {prompt_name}. Пропуск отправки системного запроса.")
        except Exception as e:
            logger.exception(f"Ошибка при отправке системного запроса: {e}")
